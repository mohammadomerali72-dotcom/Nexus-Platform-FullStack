...
export const EntrepreneursPage: React.FC = () => {
  ...
  const loadEntrepreneurs = async () => {
    try {
      setIsLoading(true)
      const entrepreneursData = await userService.getUsersByRole("entrepreneur")
      setEntrepreneurs(entrepreneursData as unknown as Entrepreneur[])
    } catch (error) {
      console.error("[v0] Failed to load entrepreneurs:", error)
      setEntrepreneurs([])
    } finally {
      setIsLoading(false)
    }
  ...
  const filteredEntrepreneurs = entrepreneurs.filter((entrepreneur) => {
    ...
    const matchesFunding =
      ...
      selectedFundingRange.some((range) => {
        ...
        const amount = Number.parseInt(fundingNeeded.replace(/[^0-9]/g, ""))
        switch (range) {
          case "< $500K":
            return amount < 500
          case "$500K - $1M":
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">{isLoading ? "..." : filteredEntrepreneurs.length} results</span>
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mt-2">Loading startups...</p>
            </div>
          ) : (
